...
if(whichConfig == 1) {
    ...
    // await $`sed -i '/FRAMEWORK_PROVIDER=/ s/test/rest/' ./shop/.env`
    ...
    // await $`sed -i '/FRAMEWORK_PROVIDER=/ s/test/graphql/' ./shop/.env`
...
await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "unzip /var/www/ganjamill/frontend.zip -d /var/www/ganjamill";`;
